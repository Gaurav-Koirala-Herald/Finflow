...
namespace FinFlowAPI.Services.Auth
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(CommonService commonService, ApplicationDbContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public CommonResponseDTO VerifyOtp(string email, string otp)
        {
            var user = _context.Users.FirstOrDefault(u => u.Email == email && u.otp == otp && u.CreatedAt.AddSeconds(60) > DateTime.UtcNow);
            if (user != null)
            {
                user.IsActive = true;
                ...
                return new CommonResponseDTO { code = HttpStatusCode.OK, message = "OTP verified successfully" };
            }
            return new CommonResponseDTO { code = HttpStatusCode.BadRequest, message = "Invalid OTP or email" };
        }
